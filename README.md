...
functions:
  ...
  # API Endpoints
  books-register:
    handler: src/endpoints/books/create.create #Path to function
...
# Background Function
  books-consumer:
    handler: src/books/functions/worker/handler.worker #Path to function
    events:
...
curl -X POST \
    -H "Content-Type: application/json" \
    -d '{"title": "American Gods", "author": "Neil Gaiman", "price": 10.00  }' \
    http://localhost:3000/services/books -i
```
...
```bash
curl -X GET \
    http://localhost:3000/services/books
```
...
```bash
curl -X GET \
    http://localhost:3000/services/books/456c9e8f-6c50-d656-dc69-dc828c42af65
```
